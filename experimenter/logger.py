...
class Logger:
    ...
    def log_step(self, state: State, simulation_time: float, step_count: int):
        ...
        if obs:
            print(f"[Step {step_count:6d} | Time {simulation_time:7.2f}s] "
                  ...
                  f"Sound Energy: {obs.sound_energy:8.2f} J | "
    ...
    def log_final(self, soundflower):
        ...
        print(f"  Total reward: {soundflower.cumulative_reward:.2f}")
        print(f"  Cumulative sound energy: {soundflower.cumulative_sound_energy:.2f} J")
        print("=" * 60)
