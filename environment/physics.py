...
class SoundPropagation:
    ...
    def compute_sound_energy(self, source_pos: np.ndarray, source_strength: float,
                            receiver_pos: np.ndarray) -> float:
        """
        Compute sound energy at receiver position using inverse square law.
        ...
        Returns:
            sound_energy: Sound energy at receiver
        """
        ...
        # Inverse square law: energy ∝ 1/r^2
        sound_energy = self.attenuation_coeff * source_strength / (distance ** 2)
        
        return sound_energy
    
    def compute_sound_energy_multiple_sources(self, source_positions: List[np.ndarray],
                                             source_strengths: List[float],
                                             receiver_pos: np.ndarray) -> float:
        """
        Compute total sound energy from multiple sources (additive).
        ...
        Returns:
            total_sound_energy: Total sound energy at receiver
        """
        total_energy = 0.0
        for pos, strength in zip(source_positions, source_strengths):
            total_energy += self.compute_sound_energy(pos, strength, receiver_pos)
        return total_energy
